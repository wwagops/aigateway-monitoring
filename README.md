...
Les états sont **historisés dans PostgreSQL** et exposés via une **API REST** et un endpoint
**Prometheus `/metrics`**, servis par le même daemon.
...
env AIGW_*  ─┘                                          │
                    ...
                    ┌──────────────┴───────────────┐
                    ▼                               ▼
            PostgreSQL (historique)         Prometheus gauges
                    ▲                               ▲
...
cp .env.example .env                     # renseigner les secrets (ACME_API_KEY, …)
```
...
- **Clés API** : référencées par `api_key_env` (nom de variable), résolues depuis
  l'environnement au runtime, envoyées dans l'en-tête `Authorization` des appels sortants.
  **Jamais** persistées, **jamais** renvoyées par l'API, **jamais** en label Prometheus.
